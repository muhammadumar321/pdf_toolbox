package com.pdftoolbox.app.ui.tools.unlock
...
class UnlockViewModel : ViewModel() {
    ...
    val unlockState = MutableStateFlow<Result<Unit>?>(null)

    fun selectFile(uri: Uri) {
        selectedFile.value = uri
    }

    fun applyUnlock(password: String, outputUri: Uri) {
        val uri = selectedFile.value ?: return
        viewModelScope.launch {
            val success = repository.unlockPdf(uri, password, outputUri)
            if (success) unlockState.value = Result.success(Unit)
            else unlockState.value = Result.failure(Exception("Failed to unlock PDF. check password."))
        }
    }
}
...
class UnlockFragment : Fragment() {
    ...
    private val createFileLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        if (result.resultCode == Activity.RESULT_OK) {
            result.data?.data?.let {
                ...
                viewModel.applyUnlock(binding.etPassword.text.toString(), it)
            }
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnAction.text = "Unlock PDF"
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.unlockState.collect { result ->
                result?.let {
                    binding.progressBar.visibility = View.GONE
                    it.onSuccess { Toast.makeText(context, "Unlocked successfully", Toast.LENGTH_SHORT).show() }
    ...
    private fun createOutputFile() {
        val intent = Intent(Intent.ACTION_CREATE_DOCUMENT).apply {
            addCategory(Intent.CATEGORY_OPENABLE)
            type = "application/pdf"
            putExtra(Intent.EXTRA_TITLE, "unlocked.pdf")
        }
